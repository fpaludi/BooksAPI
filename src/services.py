...
def signin(username, password, password2):
...
def get_books_by(column_name, value):
...
def get_books_by_id(id_value):
    book = Books.query.get(id_value)
    book = [] if book is None else book
    return book


def insert_book_review(book, user, review_value, review_comment):
